...
export const addWidgetModal = async (windowId, widgetId, modalType, windows, setWindows, currentProjectName, serverModalURL) => {
  ...
  // Check if the modal type already exists for this widget
  const existingModals = window.widgets[widgetIndex].modals || [];
  if (existingModals.includes(modalType)) {
    alert(`A ${modalType} modal already exists for this widget.`);
    return;
  }
